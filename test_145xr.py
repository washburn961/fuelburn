...
prop = Propulsion(
    BPR=5,                    # Bypass ratio
    Tmax_sl_total=2*39670,     # Total static thrust at sea level [N] (~54,000 lbf per engine x 2)
    TSFC_sl=2e-5              # Sea-level TSFC [kg/(N·s)] (~0.33 lb/(lbf·hr))
)

# Speed Profiles
climb_prof = ClimbProfile(
    initial_speed_kcas=240.0,       # 250 KCAS below 10,000 ft
    crossover_speed_kcas=270.0,     # 280 KCAS at crossover
    crossover_mach=0.76             # M0.76 above crossover
)

cruise_prof = CruiseProfile(
    altitude_ft=34000.0,            # FL320
    mach=0.76                       # M0.76 cruise
)
...
# ---------- Mission Setup ----------
initial_mass_kg = 20000.0       # Initial mass [kg] (~160,000 lb)
initial_altitude_m = 0.0      # Start at 0 m (~0 ft) after takeoff
cruise_distance_m = 1111e3      # Total mission distance: 1,111 km (~600 nm)
...
results = sim.run(
    ...
    max_time_s=14400.0  # 4 hours max
)
